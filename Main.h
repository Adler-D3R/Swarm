...
#include <arpa/inet.h> //Communication
...
//Set-up DNS Header And Packet
void DNSHeaderCreate(dns_hdr* dns)
{
...
{
     time_t t_exit = time(NULL);
     struct tm tm = *localtime(&t_exit);
     signal(sig, SIG_IGN);
     printf("\n%s [%02d:%02d:%02d] [%sEXITING%s] Swarm is exiting...\n\n", COLOR_WHITE, tm.tm_hour, tm.tm_min, tm.tm_sec, COLOR_YELLOW, COLOR_NORMAL);
     sleep(1);    
     exit(0);
}
